...
/*
Ptr<Packet>
xor_packets(const Ptr<Packet> & packet_a, const Ptr<Packet> & packet_b)
{
...
private:
  ...
  ///
  // queue for A
  queue< Ptr<Packet> > queue_a;
  // queue for B
  queue < Ptr<Packet> > queue_b;
  // Mac address that keep track of the previous, will need to generalize this
  // in the future
  Mac48Address previous_mac;
  Mac48Address mac_addr_a;
  Mac48Address mac_addr_b;
  int previous_mac_flag;
  int mac_addr_a_flag;
  int mac_addr_b_flag;
  int enable_periodic_broadcast;
  int handshake_count_a;
  int handshake_count_b;
  int broadcast_counter;
  
  Time m_broadcast_interval;
  Ptr<Packet> xor_packets (const Ptr<Packet> & a, const Ptr<Packet> & b);
  void initiate_periodic_broadcast ();
